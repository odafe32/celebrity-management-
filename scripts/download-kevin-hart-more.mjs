// Download additional freely-licensed Kevin Hart images from Wikimedia Commons
import fs from "node:fs/promises";
import path from "node:path";

const DEST_DIR = "public/images/celebrities";
const SLUG = "kevin-hart";
const START_INDEX = 3; // we already have gallery-1..2, start at 3
...
async function searchImages() {
  const url = `${COMMONS_API}?action=query&format=json&generator=search&gsrnamespace=6&gsrsearch=Kevin%20Hart&gsrlimit=40&prop=imageinfo&iiprop=url|extmetadata|mime|size&iiurlwidth=800`;
...
async function main() {
  console.log("Searching Wikimedia Commons for Kevin Hart images...");
  ...
  console.log(`\nDone. Downloaded ${downloaded} new Kevin Hart images.`);
}
